...
def process_sparse_index_group_batch(k):
    # print("hi")
...
class DLRM_Net(nn.Module):
    ...
    def sequential_forward(self, dense_x, lS_o, lS_i):
        ...
        # def process_sparse_index_group_batch(k, sparse_index_group_batch):
        #     otp_bytes = encryptor.update(secrets.token_bytes(size))
        #     otp = np.frombuffer(otp_bytes, dtype=np.uint8).reshape(arraysize)
        #     W = self.emb_l[k].weight.data + otp.astype(np.float32)
        #     encryptor = cipher.encryptor()
        key = secrets.token_bytes(32)
            # # Generate a random IV (Initialization Vector)
        iv = secrets.token_bytes(16)
        ...
        encryptor = cipher.encryptor()
        E = self.emb_l[0]
        arraysize=E.weight.data.shape
        size = np.prod(arraysize)
        arraytype=E.weight.data.dtype
        # num_cores = multiprocessing.cpu_count()
        start_cpu_part = time.process_time()

        # with Pool() as pool:
        # # call the same function with different data in parallel
        #     self_list=[self.emb_l]
        #     pool.map(process_sparse_index_group_batch, range(len(self.emb_l)))
        #         #emb_l =results
        # for k in range(len(self.emb_l)):
        #     self.emb_l[k].weight.data = self.emb_l[k].weight.data + 10

        # # Parallel(n_jobs=num_cores)(delayed(process_sparse_index_group_batch)(k, sparse_index_group_batch) for k, sparse_index_group_batch in enumerate(lS_i))
        for k, sparse_index_group_batch in enumerate(lS_i):
            otp_bytes = encryptor.update(secrets.token_bytes(size))
            otp = np.frombuffer(otp_bytes, dtype=np.uint8).reshape(arraysize)
            W = self.emb_l[k].weight.data + otp.astype(np.float32)
